...
#from models.pwc_net.correlation_package.correlation import Correlation
import numpy as np
...
def Correlation(pad_size,kernel_size,max_displacement,stride1,stride2,corr_multiply):
    k_size = int((max_displacement/stride1)*2 +1)
    corr_unfold = torch.nn.Unfold((k_size,k_size),dilation=(stride1,stride1),padding=4)
...
class PWCDCNet(nn.Module):
    ...
    def __init__(self, md=4):
        ...
        self.corr = Correlation(pad_size=md, kernel_size=1,
                                max_displacement=md, stride1=1, stride2=2,#1,
                                corr_multiply=1)
    ...
    def warp(self, x, flo):
        ...
        # scale grid to [-1,1]
        vgrid[:, 0, :, :] = 2.0 * vgrid[:, 0, :, :].clone() / max(W - 1,
                                                                  1) - 1.0
        vgrid[:, 1, :, :] = 2.0 * vgrid[:, 1, :, :].clone() / max(H - 1,
                                                                  1) - 1.0
